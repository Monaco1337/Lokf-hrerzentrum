/**
 * /crm/communication/email — E-Mail-Inbox.
 *
 * Listet E-Mail-Threads (Lead × jüngste Nachricht), klickbar auf Lead Command Center.
...
export default async function EmailInboxPage() {
  ...
  return (
    <div className="space-y-5">
      <header>
        <p className="ops-eyebrow">Kommunikation · E-Mail</p>
        ...
        <ul className="overflow-hidden rounded-xl border border-white/[0.06] bg-[#0d0d0f] divide-y divide-white/[0.05]">
          {list.map((t) => (
            <li key={t.leadId}>
              ...
              >
                <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-[12px] font-bold text-black">
                  {t.leadName
                    .split(/\s+/)
                    .map((p) => p[0])
                    .filter(Boolean)
